...
5.  Add user to group? usermod -aG`,VS=`# Linux Commands: Find All SUID Binaries (for Privilege Escalation)
...
This command turns experience into a searchable database. The longer you work in Linux, the more powerful it becomes.`,QS=`# Reflections on My Tenure at Iskaan
...
Sincerely,  
[Your Full Name]`,FS=`# Run Claude Code Locally on Apple Silicon Using LM Studio and LiteLLM (Zero Cost)
...
\`\`\`
ssh -vvv user@server
\`\`\``,ZS=`# Setting a Login Banner on Linux Servers 
...
\`\`\``,KS=`# What SRE Interviewers Really Look For
...
> "Summarize the following meeting transcript into key decisions, action items, and important discussion points and generate a MoM for this."`,PS=`# Using Manga-Downloader on Mac
...
Check flags on how to use CLI properly from the documentation.`,$S=`# DevOps Commands Cheat Sheet (With Real-World Examples)
...
💡 Why it matters: Every automation leaves behind readable, shareable context—no more "tribal knowledge."`,e1=`# How to Write Markdown .md Files in Your Code and Push It to GitHub Pages (Deployment)
...
to publish the changes to the GitHub repo (main branch) and it will automatically push the new build to the gh-pages branch (where you can check the deployment status).`,n1=`Terminal Tools For Mac OS and Linux
==============
...
nyancat in action`,t1=`# ✏️ Start writing
